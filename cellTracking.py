...
"""
function : get the date for updating, either as todate from the machine, or the date user type in
input    : N/A
output   : date
"""   
def getDate(typeErrorMess,ValueErrorMess):
    ...
    while True:
        choice = getChoice("Do you want to use todate date {}{}{} (yyyy-mm-dd) ?\n".format(bold,todate,reset),typeErrorMess,ValueErrorMess)
        if choice in "Yy":
            break
        date   = int(takeInput("Please enter a date:\n",
        ...
        year = int(takeInput("Please enter a year:\n",
                        ...
                        ,ValueErrorMess))
        try:
            todate = datetime.date(year,month,date)
        except:
            print ("We are sorry but the date is out of range for month, pleaes provide the date again\n")
...
def createRoot(typeErrorMess,ValueErrorMess):   
    ...
    while True:
        todate  = getDate(typeErrorMess,ValueErrorMess)
        ...
        choice = getChoice("Are you sure to create a root that has \nname as {} {}{},\nod as {} {}{},\nvolume as {} {}{},\nmedia as {} {}{},\nand date as {} {}{}? :\n".format(
                bold,name,reset,bold,od,reset,bold,volume,reset,bold,media,reset,bold,todate,reset),
                           typeErrorMess,ValueErrorMess)

        if choice in "yY":
            root  = Cell(name=name,od=od,volume=volume,date=todate,media=media)
            break
...
def visualization(root,typeErrorMess,ValueErrorMess):
    ...
    DG = nx.DiGraph()
    dates_dictionary = {}
    dates            = ["{}-{}-{}".format(k.year,k.month,k.date) for k in root.date]
    starting        = "{}({})".format(root.name,dates[0])
    def dfs(node):
        if node:
            ...
            for i in range(1,len(dates)):
                ...
                addDay(dates_dictionary,node.date[i],currentName)
            ...
            if node == root:
                addDay(dates_dictionary,node.date[0],"{}({})".format(name,dates[0]))
            ...
            if node.children:
                ...
                for c in node.children:
                    ...
                    cDays = ["{}-{}-{}".format(k.year,k.month,k.date) for k in c.date]        
                    cName = "{}({})".format(c.name,cDays[0])
                    dates.append(cName)
                if date in dates_dictionary:
                    dates_dictionary[date].append(dates)
                else:
                    dates_dictionary[date]=[dates]
...
def update(root,names,leaves,typeErrorMess,ValueErrorMess):
    ...
    while True:
        ...
        choice = getChoice("Are you sure to update culture {} {}{} with od {} {}{} and date {} {}{}? :\n".format(
                bold,name,reset,bold,od,reset,bold,todate,reset),
                           typeErrorMess,ValueErrorMess)
        if choice in "yY":
            break
        else:
...
#            print ( sum(newVolumes),volume)
        if sum(newVolumes) <=  volume: 
            ...
            choice = getChoice("Are you sure to dilute culture {} {}{} as follow {} {}{} with date as {} {}{}? :\n".format(
                    bold,name,reset,bold,result,reset,bold,todate,reset),
                               typeErrorMess,ValueErrorMess)
            if choice in "Yy":
                leaves.remove(name)
    ...
    targetNode.setChildren(ods = newOds,volumes= newVolumes,dateObserve =todate)
